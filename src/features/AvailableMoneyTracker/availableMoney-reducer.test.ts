...
test("current money sum will be increased when income added", () => {
    let newState = availableMoneyReducer(initState, addIncomeAC(depositId,  "Deposit",  5000,  ))    ;
...
test("current money sum will be decreased when spending added", () => {
    let newState = availableMoneyReducer(initState, addSpendAC(petId,  "Pet",  10000));
